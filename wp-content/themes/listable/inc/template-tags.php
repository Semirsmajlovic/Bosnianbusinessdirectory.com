...
function display_average_listing_rating( $post_id = null, $decimals = 2 ) {
	...
	<a href="#comments" class="bbd-button single-rating review_rate display-only" data-pixrating="<?php echo $rating ?>" itemprop="aggregateRating" itemscope="" itemtype="http://schema.org/AggregateRating">
...
function listable_display_frontpage_listing_types($default_count = 7) {
    $term_list = array();
    ...
    //now create an array with the type slug as key so we can reference/search easier
    $all_types = array();
    foreach ( $all_terms as $key => $term ) {
        $all_types[ $term->slug ] = $term;
    }

    $types             = get_post_meta( get_the_ID(), 'frontpage_listing_types', true );
    $custom_type_labels = array();

    //if we have received a list of types to display (their slugs and optional label), use that
    if ( ! empty( $types ) ) {
        $types = explode( ',', $types );
        foreach ( $types as $key => $type ) {
            if ( strpos( $type, '(' ) !== false ) {
                $type  = explode( '(', $type );
                $term_slug = trim( $type[0] );
                $term_slug = sanitize_title_for_query( $term_slug );

                if ( substr( $type[1], - 1, 1 ) == ')' ) {
                    $custom_type_labels[ $term_slug ] = trim( substr( $type[1], 0, - 1 ) );
                }

                if ( array_key_exists( $term_slug, $all_types ) ) {
                    $term_list[] = $all_types[ $term_slug ];
                }
            } else {
                $term_slug   = trim( $type );
                $term_slug = sanitize_title_for_query( $term_slug );
        ...
        //it seems we will have to figure out ourselves what types to display

        $term_list = array_slice( $all_types, 0, $default_count);
    }
    ...
    foreach ( $term_list as $key => $term ) :
        ...
        <a href="<?php echo esc_url( get_term_link( $term ) ); ?>">
            ...
            <span class="cat__text"><?php echo isset( $custom_type_labels[ $term->slug ] ) ? $custom_type_labels[ $term->slug ] : $term->name; ?></span>
        </a>

    <?php endforeach;

    $cta = get_post_meta( get_the_ID(), 'frontpage_listing_types_cta', true );
    if ( ! empty( $cta ) && $term_list ) {
        echo '<div style="position: relative;"><span class="cta-text">' . $cta . '</span></div>';
    }
}
...
function listable_display_frontpage_listing_categories( $default_count = 7 ) {
	...
	$cta = get_post_meta( get_the_ID(), 'frontpage_listing_categories_cta', true );
	if ( ! empty( $cta ) && $term_list ) {
		echo '<div style="position: relative;"><span class="cta-text">' . $cta . '</span></div>';
	}
}
