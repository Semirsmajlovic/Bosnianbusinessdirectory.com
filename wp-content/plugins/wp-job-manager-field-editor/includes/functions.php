...
if( ! function_exists( 'get_company_id_from_job' ) ){
	...
	function get_company_id_from_job( $job_id = false ){
		...
		} elseif( ! empty( $job_id ) && $post_type === 'company' ){
			$company_id = $job_id;
		}
...
if ( ! function_exists( 'get_custom_field_listing_meta' ) ){
	...
	function get_custom_field_listing_meta( $field_slug, $listing_id, $args = array() ){
		...
		if( $field_slug === 'job_title' || $field_slug === 'candidate_name' || ( $post_type === 'company' && $field_slug === 'company_name' ) ) return apply_filters( 'field_listing_title', get_the_title( $listing_id ) );
		if( $field_slug === 'job_description' || $field_slug === 'resume_content' || ( $post_type === 'company' && $field_slug === 'company_content' ) ) return apply_filters( 'field_listing_content', get_the_content( $listing_id ) );
