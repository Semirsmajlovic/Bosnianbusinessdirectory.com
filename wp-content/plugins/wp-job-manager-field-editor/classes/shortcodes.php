...
class WP_Job_Manager_Field_Editor_ShortCodes {
	...
	function shortcode_output( $atts, $content = '', $tag = 'jmfe' ) {
		...
		// No value set for listing_id, let's try and use query object to get ID
			...
			if ( is_object( $qo ) && isset( $qo->ID ) ) {
				...
				$shortcode_post_types = apply_filters( 'job_manager_field_editor_shortcode_output_post_types', array( 'job_listing', 'resume', 'company' ) );
	...
	public function get_listing_ID( $atts = array() ){
		...
		$shortcode_post_types = apply_filters( 'job_manager_field_editor_shortcode_output_post_types', array( 'job_listing', 'resume', 'company' ), $this );
