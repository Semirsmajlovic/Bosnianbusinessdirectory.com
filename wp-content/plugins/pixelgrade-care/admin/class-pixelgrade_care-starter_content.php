...
class PixelgradeCare_StarterContent {
	...
	public function __construct( $parent ) {
		...
		if ( apply_filters( 'pixcare_sce_allow_options_filtering', true ) ) {
			 ...
			 */
			add_filter( 'pixcare_sce_import_post_theme_mod_custom_logo', array( $this, 'filter_post_theme_mod_custom_logo' ), 10, 2 );
			/**
			 * Some themes use custom keys for various attachment ID controls, so we need to treat them separately.
			 */
			add_filter( 'pixcare_sce_import_post_theme_mod_rosa_transparent_logo', array( $this, 'filter_post_theme_mod_custom_logo' ), 10, 2 );
			add_filter( 'pixcare_sce_import_post_theme_mod_osteria_transparent_logo', array( $this, 'filter_post_theme_mod_custom_logo' ), 10, 2 );
			add_filter( 'pixcare_sce_import_post_theme_mod_pixelgrade_transparent_logo', array( $this, 'filter_post_theme_mod_custom_logo' ), 10, 2 );
			...
			// content links
			add_action( 'pixcare_sce_after_insert_post', array( $this, 'prepare_menus_links' ), 10, 3 );
	...
	public function rest_upload_media( $request ) {
		...
		if ( ! is_wp_error( $attachment_id ) ) {
			...
			if ( ! empty( $starter_content[ $demo_key ]['media'][ $group ][ $remote_id ] ) ) {
				...
				if ( ! empty( $previous_attachment_metadata['imported_with_pixcare'] ) ) {
	...
	private function import_post_type( $demo_key, $base_url, $args = array() ) {
		...
		foreach ( $response_data['data']['posts'] as $i => $post ) {
			...
			if ( $existing_post_id = $this->the_slug_exists( $post['post_name'], $post['post_type'] ) ) {
				...
				if ( apply_filters( 'pixcare_sce_should_overwrite_existing_post', false, $existing_post_id, $post, $demo_key ) ) {
			...
			// Since there are some theme versions that do wp_slash() for mainly post_content, we need to remove their filters,
			// at least for a while @todo remove this after 01.06.2020
			if ( has_filter( 'pixcare_sce_insert_post_args', 'rosa2_adjust_pixcare_sce_import_post_args' ) ) {
				remove_filter( 'pixcare_sce_insert_post_args', 'rosa2_adjust_pixcare_sce_import_post_args', 10 );
			}

			// Allow others to have a say in it.
			$post_args = apply_filters( 'pixcare_sce_insert_post_args', $post_args, $post, $demo_key );
		...
		foreach ( $response_data['data']['posts'] as $i => $post ) {
			...
			do_action( 'pixcare_sce_after_insert_post', $post, $imported_ids, $demo_key );
		}
	...
	private function import_taxonomy( $demo_key, $base_url, $args ) {
		...
		if ( ! taxonomy_exists( $args['tax'] ) ) {
			return rest_ensure_response( array(
				...
				'message' => sprintf( esc_html__( '%s does not exists!', 'pixelgrade_care' ), $args['tax'] ),
				'data'    => array(),
			) );
		}
	...
	private function import_settings( $demo_key, $type, $data ) {
		...
		if ( ! empty( $data['mods'] ) ) {
			...
			foreach ( $data['mods'] as $mod => $value ) {
				...
				if ( is_array( $value ) ) {
					$value = array_map( array( $this, 'sanitize_import_value' ), $value );
				} else {
					$value = $this->sanitize_import_value( $value );
				}

				$value = apply_filters( "pixcare_sce_import_{$type}_theme_mod_{$mod}", $value, $demo_key );
		...
		if ( ! empty( $data['options'] ) ) {
			...
			foreach ( $data['options'] as $option => $value ) {
				...
				$value = apply_filters( "pixcare_sce_import_{$type}_option_{$option}", $value, $demo_key );
