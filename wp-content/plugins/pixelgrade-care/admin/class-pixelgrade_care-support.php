...
class PixelgradeCare_Support {
	...
	public function support_setup() {
		...
		wp_enqueue_style( $this->parent->get_plugin_name(), plugin_dir_url( $this->parent->file ) . 'admin/css/pixelgrade_care-admin' . $rtl_suffix . '.css', array(), $this->parent->get_version(), 'all' );
		...
		wp_enqueue_script( $this->parent->get_plugin_name() . '-support', plugin_dir_url( $this->parent->file ) . 'admin/js/support' . $suffix . '.js', array(
			'jquery',
			'wp-util',
			'wp-a11y',
			'updates',
		...
		if ( ! wp_script_is('pixelgrade_care-dashboard') ) {
			PixelgradeCare_Admin::localize_js_data( 'pixelgrade_care-support', true, 'support' );
		}

		// We need to remove lodash from the global scope since it overwrites underscores and everything breaks in the Customizer.
		$lodash_noconflict = '
